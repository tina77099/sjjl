...
function createEventElement(event) {
    ...
    // 获取并渲染真正的标签
    let tagsHtml = '';
    if (event.tags && Array.isArray(event.tags) && event.tags.length > 0 && window.tagsManager) {
        try {
            const allTags = window.tagsManager.getTags();
            const eventTags = event.tags.map(tagId => 
                allTags.find(tag => tag.id === tagId)
            ).filter(Boolean); // 过滤掉不存在的标签
            
            if (eventTags.length > 0) {
                tagsHtml = eventTags.map(tag => {
                    return `<span class="ml-2 px-2 py-0.5 text-xs rounded-full text-white" style="background-color: ${tag.color}">${tag.name}</span>`;
                }).join('');
            }
        } catch (error) {
...
function createEventCardElement(event) {
    ...
    if (!tagsHtml && event.category) {
        ...
        categoryBadgeHtml = `
            <div class="mt-2 flex flex-wrap gap-1">
                <span class="px-2 py-0.5 bg-gray-100 text-gray-800 text-xs rounded-full">${categoryName}</span>
            </div>
        `;
    }
