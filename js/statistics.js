...
function initCategoryChartWithData(categoryCtx, data) {
    ...
    // 更新图例文本
    const legendContainer = document.querySelector('.grid.grid-cols-1.gap-4.mt-6');
    if (legendContainer) {
        // 清空现有图例
        legendContainer.innerHTML = '';
        
        // 计算每行应显示的图例数量（将图例平均分为两行）
        const legendsPerRow = Math.ceil(categories.length / 2);
        
        // 创建两行容器
        const row1 = document.createElement('div');
        row1.className = 'col-span-2 grid grid-cols-3 gap-2';
        
        const row2 = document.createElement('div');
        row2.className = 'col-span-2 grid grid-cols-3 gap-2';
        ...
        categories.forEach((category, index) => {
            ...
            colorIndicator.style.backgroundColor = backgroundColors[index].replace('0.8', '1');
            
            const legendText = document.createElement('span');
            legendText.className = 'text-sm text-gray-600 truncate';
            legendText.textContent = `${getCategoryDisplayName(category)} (${percentages[index]}%)`;
            
            legendItem.appendChild(colorIndicator);
            legendItem.appendChild(legendText);
            
            // 根据索引分配到第一行或第二行
            if (index < legendsPerRow) {
                row1.appendChild(legendItem);
            } else {
                row2.appendChild(legendItem);
            }
        });
